...
class EstatResult(BaseModel):
    ...
    stat_price: float = Field(None, description="e-Statから取得した平均価格")
    stat_unit: str = Field(None, description="統計価格の単位（例: 100g, 1パック）")
    diff: float = Field(None, description="支払価格と統計価格の差額")
    rate: float = Field(None, description="統計価格に対する乖離率（0.1 = +10%）")
    judgement: str = Field("UNKNOWN", description="価格の妥当性判断（DEAL, OVERPAY, FAIR, UNKNOWN）")
    note: str = Field(None, description="補足情報やエラー理由")
...
class ItemResult(BaseModel):
    ...
    canonical: str = Field(None, description="名寄せ後の標準的な商品名")
    paid_unit_price: float = Field(None, description="レシートから抽出した支払単価")
...
class CanonicalResolution(BaseModel):
    canonical: str = Field(None, description="解決された標準名称")
    class_id: str = Field(None, description="e-StatのカテゴリID")
    class_code: str = Field(None, description="e-Statの項目コード")
