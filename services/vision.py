...
def analyze_receipt_with_market_data(file_bytes: bytes, market_data: list[dict]) -> dict:
    """画像と市場データをGeminiに送り、詳細な分析結果をJSONで受け取ります。"""
    ...
    try:
        ...
        prompt = SYSTEM_INSTRUCTION.replace("{{MARKET_DATA_JSON}}", market_data_json)
        ...
        if not response.text:
             raise HTTPException(status_code=500, detail="Gemini APIから応答がありませんでした。")
...
def extract_text_from_image(file_bytes: bytes) -> str:
    # 古い関数：新しい analyze_receipt_with_market_data への移行を推奨
